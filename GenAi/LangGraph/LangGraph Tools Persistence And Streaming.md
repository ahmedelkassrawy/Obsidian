---
description: "Large pasted LangGraph reference: defining tools and schema, nodes/routers/edges, a ReAct agent, persistence with interruptions, streaming, and threads."
domain: ai-eng
type: reference
status: raw
tags:
...
Merged from the old `LangGraph Tool Node Basics` stub. Rule of thumb: **don't take `state` as a tool parameter** — always use a `ToolNode` and bind the tools to the LLM. When the model node returns, it can preserve the other state fields alongside the new message:
