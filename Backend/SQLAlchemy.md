...
All async DB actions use `AsyncSession` and `select()`, and every write is `await`ed.
...
- Creates an **async session factory** so `async with AsyncSessionLocal()` opens a DB connection asynchronously.
- Keep `expire_on_commit=False` for async sessions (avoids lazy-load errors after commit).
...
- Yields one **AsyncSession** per request so requests don't block each other.
...
```
- **Library:** `sqlalchemy` — one ORM result or `None`.
- Alternatives: `.scalars().all()` → a list; `.scalar_one()` → expect exactly one (errors otherwise).
...
db.add(new_record)
```
- **Library:** `sqlalchemy` — stages the object; it's inserted on `commit()`.
...
await db.commit()
```
- **Library:** `sqlalchemy.ext.asyncio` — finalizes INSERT/UPDATE/DELETE. Always `await db.commit()` or the changes aren't persisted.
...
```
- **Library:** `sqlalchemy.ext.asyncio` — reloads the object (e.g. to get autogenerated IDs).
...
await db.commit()
```
- Modify the field on the ORM object, then commit.
...
```
- **Library:** `fastapi` + `sqlalchemy.ext.asyncio` — runs DB logic at app startup (e.g. pre-populate if empty).
...
> In short: all async DB actions come from SQLAlchemy's async API (`sqlalchemy.ext.asyncio`) and use `await` to avoid blocking FastAPI's event loop.
...
- **Async**: Use `sqlalchemy.ext.asyncio` (`AsyncSession`, `async_sessionmaker`) with `await` for non-blocking DB ops in async FastAPI apps.
